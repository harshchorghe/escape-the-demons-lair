...
export class EffectsEngine {
  ...
  private wisteriaSystem!: THREE.Points;
  private wisteriaVelocities: PetalVelocity[] = [];
  ...
  buildWisteriaPetals() {
    ...
    const velocities: PetalVelocity[] = [];
  ...
  spawnBreathingEffect(style: string, position: THREE.Vector3, direction: THREE.Vector3) {
    if (style === 'water') {
  ...
  spawnWaterWave(pos: THREE.Vector3, dir: THREE.Vector3) {
    const count = 60;
  ...
  spawnFlameBurst(pos: THREE.Vector3, dir: THREE.Vector3) {
    const count = 75;
    for (let i = 0; i < count; i++) {
  ...
  spawnThunderSparks(pos: THREE.Vector3, dir: THREE.Vector3) {
    const count = 80;
    for (let i = 0; i < count; i++) {
  ...
  spawnDamageNumber(amount: number, position: THREE.Vector3, isCritical: boolean = false) {
  ...
  update(deltaTime: number) {
    if (!this.wisteriaSystem) return;
    // Update Wisteria Petals
    const positions = (this.wisteriaSystem.geometry.attributes.position as THREE.BufferAttribute).array as Float32Array;
  ...
  dispose() {
    ...
    this.particles = [];
    if (this.wisteriaSystem) {
      this.scene.remove(this.wisteriaSystem);
      this.wisteriaSystem.geometry.dispose();
      (this.wisteriaSystem.material as THREE.Material).dispose();
    }
  }
}
