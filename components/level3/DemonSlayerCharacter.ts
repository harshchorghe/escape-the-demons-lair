...
export class DemonSlayerCharacter {
  ...
  buildHighResCharacter() {
    ...
    if (this.joints.rightArm && this.joints.katanaGrp) {
  ...
  private _buildLeg(parent: THREE.Group, xOffset: number, side: 'left' | 'right') {
    const leg = new THREE.Group();
    leg.position.set(xOffset, -0.12, 0);
    parent.add(leg);
    this.joints[`${side}Leg` as keyof JointsMap] = leg as any;
  ...
  private _buildTorso(torso: THREE.Group) {
    const chestPoints = [
  ...
  private _buildHaori(torso: THREE.Group) {
  ...
  private _addHaoriPattern(haoriGroup: THREE.Group) {
    const cols = 4, rows = 5;
  ...
  private _buildHead(headGroup: THREE.Group) {
    ...
    this._buildEye(headGroup, 0.088, 0.215);
    this._buildEye(headGroup, -0.088, 0.215);
  ...
  private _buildEye(parent: THREE.Group, xPos: number, yPos: number) {
    const zPos = 0.205;
  ...
  private _buildEyebrow(parent: THREE.Group, xPos: number, yPos: number) {
  ...
  private _buildScar(parent: THREE.Group) {
  ...
  private _buildHair(parent: THREE.Group) {
    ...
    const spikeConfigs: { pos: [number, number, number]; rot: [number, number, number]; scale: [number, number, number] }[] = [
    ...
    const bangConfigs: { pos: [number, number, number]; rot: [number, number, number]; scale: [number, number, number] }[] = [
  ...
  private _buildArm(parent: THREE.Group, xOffset: number, side: 'left' | 'right') {
    const arm = new THREE.Group();
    arm.position.set(xOffset, 0.56, 0);
    parent.add(arm);
    this.joints[`${side}Arm` as keyof JointsMap] = arm as any;
  ...
  private _buildFingers(arm: THREE.Group, isLeft: boolean) {
    const fingerOffsets: [number, number, number][] = [
      [-0.03, -0.895, 0.02],
      [-0.01, -0.908, 0.025],
      [0.01, -0.908, 0.025],
      [0.03, -0.895, 0.02],
  ...
  private _buildKatana() {
    ...
    this.joints.bladeGlowMat = this.mat.bladeGlow as THREE.MeshStandardMaterial;
  ...
  private _buildOwndaysGlasses(headGroup: THREE.Group) {
  ...
  setBreathingStyle(style: string) {
    ...
    if (style === 'water') {
      (this.mat.haori as THREE.MeshStandardMaterial).color.setHex(0x0d7f7f);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).color.setHex(0x00e0ff);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).emissive.setHex(0x00c4e8);
      (this.mat.glassAccent as THREE.MeshStandardMaterial).color.setHex(0x00a8a8);
      (this.mat.eyeIris as THREE.MeshStandardMaterial).color.setHex(0xaa0000);
    } else if (style === 'flame') {
      (this.mat.haori as THREE.MeshStandardMaterial).color.setHex(0xb82000);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).color.setHex(0xff4400);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).emissive.setHex(0xdd2200);
      (this.mat.glassAccent as THREE.MeshStandardMaterial).color.setHex(0xff4500);
      (this.mat.eyeIris as THREE.MeshStandardMaterial).color.setHex(0xff4500);
    } else if (style === 'thunder') {
      (this.mat.haori as THREE.MeshStandardMaterial).color.setHex(0xd4a800);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).color.setHex(0xffee00);
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).emissive.setHex(0xddcc00);
      (this.mat.glassAccent as THREE.MeshStandardMaterial).color.setHex(0xffee00);
      (this.mat.eyeIris as THREE.MeshStandardMaterial).color.setHex(0xffe600);
    }
  }
  ...
  update(deltaTime: number, isMoving: boolean, moveDirection: THREE.Vector3) {
    this.animTime += deltaTime;
    const t = this.animTime;

    const { torso, head, leftArm, rightArm, leftLeg, rightLeg, katanaGrp, haoriBack } = this.joints;

    if (isMoving && this.state === 'idle') this.state = 'run';
    else if (!isMoving && this.state === 'run') this.state = 'idle';

    if (this.mat.bladeGlow) {
      (this.mat.bladeGlow as THREE.MeshStandardMaterial).emissiveIntensity = 1.6 + Math.sin(t * 4) * 0.6;
    }
    ...
    if (!torso || !head || !leftArm || !rightArm || !leftLeg || !rightLeg || !katanaGrp) return;
